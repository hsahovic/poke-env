...
with open("src/poke_env/data/static/learnset.json") as f:
    movesets = orjson.loads(f.read())

print("-" * 20, "\n")
mons = [mon for mon in GenData.from_gen(9).pokedex]
...
def random_mon():
    ...
    if "baseSpecies" in GenData.from_gen(9).pokedex[mon]:
        mon = GenData.from_gen(9).pokedex[mon]["baseSpecies"]
    else:
        mon = GenData.from_gen(9).pokedex[mon]["species"]

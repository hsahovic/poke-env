...
.. code-block:: python
    ...
    player = RandomPlayer(save_replays="replays")

If ``save_replays=True``, replays are stored in the default replay folder.
If ``save_replays`` is a string, that path is used.

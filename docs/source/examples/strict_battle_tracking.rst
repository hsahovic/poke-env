...
``Player`` provides ``strict_battle_tracking`` to aggressively validate internal
battle-state consistency against Showdown requests.
...
- Strict mode may raise assertions in edge cases with incomplete information.
